...
pubDate: "YYYY-MM-DD"  # 카드 생성 당일 날짜 (date +%Y-%m-%d)
---
```
...
2. **pubDate**: 카드 생성 당일 날짜 삽입 — `date +%Y-%m-%d` 실행 결과 사용 (미래 날짜 금지)
3. **title 포맷**: 명사형(개념명/인물명) 사용 필수
   - ✅ "파블로프의 개" / "양자 컴퓨터" / "알렉산더 플레밍"
   - ❌ "왜 개는 종소리에 반응하는가?" (질문형 금지)
   - ❌ "실수가 세상을 구했다" (서술 문장형 금지)
   - ❌ "저항이 사라지는 세계" (개념명 없는 은유형 금지)
   - people 카드: 반드시 인물 이름 포함
   - origins 카드: 단어/어원 자체가 title
4. **slug**: 파일명이 곧 slug (영문 kebab-case)
5. **스텝 수**: 10~16개 사이
6. **reveal-title**: 항상 마지막 2번째. `{emoji} {title}` + 한 줄 설명
7. **outro**: 항상 마지막. 여운 남기는 마무리
8. **cinematic-hook**: 항상 첫 번째. 강렬한 첫인상
9. **텍스트**: 짧은 문장. 한 줄 5~15자. `**볼드**`로 핵심 강조
10. **방식 선택**: 인물 대화 중심 → 만화 / 개념/팩트 중심 → 시네마틱
11. **coverImage**: 선택사항. 있으면 reveal-title에서 정사각 프레임 표시
